...
- **AI Report History (archive)** — Every generated Weekly Review, Journal Summary, Wellbeing Summary (Insights AI Review), and Journal Insights (AI Journal Review) is saved to a dedicated `aiReports` archive (scoped per user, soft-deleted). Each AI hub card exposes a wand icon to (re)generate and a clock icon that opens a browsable history of past reports by date; any saved report can be reopened in full or deleted. Reading history requires no Gemini call — the history stays reachable even when no API key is configured (including in the read-only demo).
