...
| Demo & Tour | Shipped | Curated 6-stop product tour (static sample data + example AI outputs), Roadmap page; read-only public demo still exists at `/?demo=1` but has no UI entry point |
...
- **Take a Tour (curated walkthrough)** — A 6-stop curated product walkthrough (Welcome → Dashboard → AI Weekly Review → Journal Intelligence → Insights → How it's built) positioned as a recruiter-facing evaluation of the product and its AI features, readable in about two minutes. Each stop pairs a short narrative with a **static preview panel** rendered from one deterministic sample week (`src/pages/tour/tourContent.ts`) — no iframe, no API calls, no Gemini key. The three AI stops show **prewritten example outputs** composed from that sample data, each explicitly labeled as an example (never implying live generation). App-view stops (Dashboard, Weekly Review, Journal AI, Insights) have a **Desktop/Mobile toggle**: Mobile re-renders the same panel single-column inside a phone frame with the app's mobile chrome (header + bottom tab bar, active tab highlighted). Reachable from the Compass button in the header, the new-user Setup Guide, and the Login screen (no account required). Renders in `app` mode (final CTA: Go to Dashboard) or pre-login `auth` mode (CTAs: Create an account / Sign in / Roadmap)
- **Public Demo Mode (read-only, no UI entry point)** — The full read-only demo infrastructure remains functional at `/?demo=1`: real screens and real server-derived views scoped to a seeded demo dataset (~10 weeks across all domains, refreshed relative to today). Writes are blocked client-side (friendly toast) and server-side (403); a persistent banner states the read-only contract with an Exit demo action. The beta Analytics/Insights pages are viewable in the demo, labeled Beta. Server flag: `PUBLIC_DEMO_ENABLED`. **The Login-screen "Explore the live demo" button has been removed** — the curated tour is the public-facing walkthrough; the demo is reachable only by direct URL. See [`DEMO_ARCHITECTURE.md`](DEMO_ARCHITECTURE.md)
