...
| AI Features (Gemini BYOK) | Shipped | Header AI hub (Wellbeing Summary, Weekly Review, Journal Insights), summaries, variant suggestions |
...
- **Bundle Removal Safety** — Removing a bundle never removes its sub-habits. The Remove Habit modal lists the affected sub-habits and offers a checkbox to archive them together with the bundle; otherwise they surface as standalone habits. Deleting a bundle parent releases its children server-side (their `bundleParentId` is cleared) so they can never be stranded invisibly; archiving keeps the links so unarchiving restores the bundle intact
...
- **Remove Habit Modal (goal-linked habits and bundles)** — Removing a habit linked to one or more goals, or a bundle with sub-habits, opens a chooser listing affected goals and sub-habits and offering two paths: **Archive** (recommended, restorable) or **Delete permanently** (soft-delete, not restorable from UI). Historical goal progress is preserved either way — past entries keep contributing to the goal — but only Archive lets you bring the habit itself back without re-creating it. For bundles, an optional checkbox archives the sub-habits along with the parent; otherwise they remain as standalone habits
- **Reordering** — Drag-and-drop to reorder habits within categories
- **Push Reminders** — Set a reminder time per habit (create/edit modal, Schedule section); a Web Push notification fires at that local time on the habit's assigned days and is skipped when the habit is already completed that day. Bundles can have reminders too: a choice bundle counts as done once any child is completed, a checklist bundle once its success rule is met. Notifications are enabled per device from **Settings → Notifications** (test-send included); on iPhone/iPad the app must be added to the Home Screen first (iOS 16.4+). Server-side the feature is gated by `PUSH_REMINDERS_ENABLED` + VAPID keys, and the in-process scheduler requires an always-on backend instance
...
- **Goal Schedule View** — Calendar showing deadlines, forecasts, milestones, and scheduled-goal start dates across all goals
...
- **Goal Lifecycle Status** — Standalone goals carry a lifecycle status: **Active** (default), **Scheduled** (planned to start later), or **Backlog** (someday/maybe). Set in the Create/Edit goal modals. On the Goals page, each category stack shows active goals first with collapsed "Scheduled (n)" / "Backlog (n)" sub-sections below; cards there carry a status chip. Tracked goals are exempt — their lifecycle is governed by the track sequence (`trackStatus`). Progress still derives from entries regardless of status, and a backlog goal whose entries hit its target still completes
- **Scheduled Start Dates** — A Scheduled goal can carry an optional start date (`YYYY-MM-DD`). When the date arrives (in the user's timezone), goal reads automatically promote it to Active — no manual step. Dated scheduled goals also appear on the Schedule calendar as a cyan **Starts** event
- **Goal Ordering** — Drag-and-drop reorder within category groups (active goals only; scheduled/backlog sub-sections are fixed-order)
...
- **AI Journal Summary Banner** — User-requested weekly summary (when Gemini API key is configured) shown as a dismissible banner on the journal page; generated on demand via a "Generate weekly summary" button, saved as a journal entry in history
...
- **AI Hub (header)** — The three primary AI artifacts — **Wellbeing Summary**, **Weekly Review**, and **Journal Insights** — are gathered in a single modal opened from the sparkle **AI** icon in the top header. Each is a full inline generator showing a brief explanation, the date it was last generated, and a clock **history** icon for browsing past reports of that kind.
- **Weekly Review** — The single, comprehensive weekly report for a selected week (this week / last week), generated from the header AI hub. It both tells the story of the week and provides evidence-based analysis. Aggregates habit entries, sleep & mood from wellbeing check-ins, journal activity, and goals into observed facts, then returns a typed review with seven sections: **Week at a Glance** (a natural-language narrative recap, 1–3 paragraphs), **Facts** (objective, measurable observations), **Patterns** (each with a low/medium/high confidence), **Journal Themes** (recurring topics and emotional trends), **Wins**, **Areas for Attention**, and **Recommendations** (max 3–5) — plus honest **Data Limitations**. The prompt separates observed facts from inferred patterns from suggestions and forbids inventing data; low-data weeks are reported honestly via Data Limitations rather than fabricated patterns. (Consolidates the former standalone AI Weekly Summary.)
- **Wellbeing Summary (Insights AI Review)** — On-demand narrative available from the header AI hub (and the Insights page's AI Review tab). Computes the user's cross-domain insights (correlations, linear-trend predictions, medication adherence, metric averages) and asks Gemini to explain them in plain language. Returns a typed review: **Summary**, **Key Findings** (objective, cited from the computed numbers), **Patterns** (each with a low/medium/high confidence), **Outlook** (caveated read of the trend predictions), **Recommendations** (max 5), and **Data Limitations**. Grounded only on already-computed facts; everything framed as correlation, never causation; the window is server-owned. Each generation is saved to history
- **AI Journal Summary** — User-requested weekly journal summary shown as a dismissible banner; generated on demand, persisted as a journal entry in history
- **AI Report History (archive)** — Every generated Weekly Review, Journal Summary, Wellbeing Summary (Insights AI Review), and Journal Insights (AI Journal Review) is saved to a dedicated `aiReports` archive (scoped per user, soft-deleted). Each AI hub card exposes a wand icon to (re)generate and a clock icon that opens a browsable, full-height history of past reports by date; any saved report can be reopened in full or deleted. Reading history requires no Gemini call — the history stays reachable even when no API key is configured (including in the read-only demo). When no key is configured, each AI hub card also renders its latest archived report inline (not just a link to history), so the read-only demo shows real AI output for the Weekly Review, Wellbeing Summary, and Journal Insights cards instead of an empty key prompt.
- **Journal Insights (AI Journal Review)** — On-demand, structured review of journal entries over a user-selected date range, available from the header AI hub (and the Journal page's AI Review tab; presets for last 7/30 days plus custom range). Grounded only in the user's own entries, it returns a typed review: Overview, Emotional Themes, Recurring Stressors, and Self-Talk Patterns (each with paraphrased evidence; themes/stressors carry a low/medium/high confidence), Wins, Reflection Questions, Suggested Next Steps, and Data Limitations. The prompt separates observed evidence from inferred themes from next steps, forbids inventing facts or long quotes, and is deliberately non-clinical (no diagnoses). Empty ranges show a helpful empty state, sparse ranges show a low-data warning, and entries suggesting crisis surface a gentle support notice rather than counseling. Each generation is saved to history; regenerate at any time
...
- **Wellbeing Card** — Wide card showing today's status (Morning / Evening / Sleep) and three actions: ☀️ Morning, 🌙 Evening, 💊 Health; chevron opens the Wellbeing Overview (which still links to the Insights page)
...
- **Journal Card** — Wide action card with three actions that deep-link to the matching Journal tab: Free, Template, History
...
> The three primary AI artifacts (Wellbeing Summary, Weekly Review, Journal Insights) are no longer scattered across the Dashboard, Habits page, and Journal/Insights tabs. They now live together in the **header AI hub** (sparkle AI icon); see AI Features.
...
- **Take a Tour (interactive)** — An 11-stop guided walkthrough of the live product (Welcome → Dashboard → Habits → Weekly AI Review → Goals → Tasks → Journal → AI Journal Review → Routines → Wellbeing Insights → What's next). Three AI features (Weekly AI Review, AI Journal Review, Wellbeing Insights) are first-class stops paired with their home domains, marked with violet "AI" chips and badges, and called out in the tour hero; the AI Routine Builder's variant-drafting flow is folded into the Routines stop instead of standing alone. Each stop pairs a grounded narrative with a **live preview**: an embedded read-only demo of the real app on seeded data, driven between views without reloading and deep-linked to the exact section each stop describes — the Weekly AI Review stop opens the AI hub showing a completed archived review on top, the Routines stop opens the Morning Kickstart routine editor with "Suggest with AI" one click away, and AI Journal Review opens on the Journal AI Review tab with the saved review already visible — plus a **Desktop/Mobile toggle** that renders true responsive layouts in a real 390px viewport. Honesty cues throughout (Functional today / Beta / Roadmap badges, BYOK and sample-report callouts). Reachable from the "Take the tour" button on the Login screen (no account required) or directly via `?view=tour`. Renders in `app` mode (CTAs jump into the app) or pre-login `auth` mode (CTAs: Create an account / Sign in / Roadmap)
- **Public Demo Mode (read-only)** — Visitors can browse the entire app without an account via `/?demo=1`; the tour's embedded live previews run on the same mechanism. (The Login screen's "Explore the live demo" button is temporarily hidden behind the `SHOW_DEMO_CTA` flag in `LoginPage.tsx` while the tour is the primary public entry point.) Real screens and real server-derived views scoped to a seeded demo dataset (~10 weeks across all domains, refreshed relative to today). Writes are blocked client-side (friendly toast) and server-side (403); a persistent banner states the read-only contract with an Exit demo action. The beta Analytics/Insights pages are viewable in the demo, labeled Beta. Server flag: `PUBLIC_DEMO_ENABLED`. See [`DEMO_ARCHITECTURE.md`](DEMO_ARCHITECTURE.md)
