...
- **Take a Tour (interactive)** — A 14-stop guided walkthrough of the live product (Welcome → Dashboard → Habits → Weekly AI Review → Goals → Tasks → Journal → AI Journal Review → Routines → AI Routine Builder → Wellbeing Insights → Sleep → Settings → What's next). The four AI features (Weekly AI Review, AI Journal Review, Wellbeing Insights, AI Routine Builder) are first-class stops paired with their home domains, marked with violet "AI" chips and badges, and called out in the tour hero. Each stop pairs a grounded narrative with a **live preview**: an embedded read-only demo of the real app on seeded data, driven between views without reloading and deep-linked to the exact section each stop describes (the Weekly AI Review stop opens the AI hub showing a completed archived review, the AI Routine Builder stop opens the routine editor, Sleep opens the Analytics Sleep tab, Settings opens the Settings modal), plus a **Desktop/Mobile toggle** that renders true responsive layouts in a real 390px viewport. Honesty cues throughout (Functional today / Beta / Roadmap badges, BYOK and sample-report callouts). Reachable from the "Take the tour" button on the Login screen (no account required) or directly via `?view=tour`. Renders in `app` mode (CTAs jump into the app) or pre-login `auth` mode (CTAs: Create an account / Sign in / Roadmap)
