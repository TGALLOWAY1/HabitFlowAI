...
| Take a Tour | Page | "Take the tour" on Login (no account required) / `?view=tour` | Interactive 11-stop guided walkthrough with three AI features as first-class stops (violet "AI" chips/badges): Weekly AI Review follows Habits, AI Journal Review follows Journal, and Wellbeing Insights leads with its AI Review narration. The AI Routine Builder is no longer a standalone stop — its variant-drafting flow is folded into the Routines stop. Each stop pairs a narrative panel (problem, mechanics, honesty badge: Functional today / Beta / AI · BYOK / Roadmap) with a live preview — a persistent iframe running the real app read-only at `/?demo=1&embed=1`, navigated between stops via postMessage. Every stop deep-links to the exact section it describes — not just a route: the Weekly AI Review stop opens the AI hub with the completed archived review on top, the Routines stop opens the Morning Kickstart routine editor with "Suggest with AI" one click away, and AI Journal Review opens on the Journal AI Review tab with the saved review already visible (control params: `modal`, `focus`, `routineEditor`, `tab`). Overlays from the previous stop close automatically on each step change. Desktop/Mobile toggle resizes the preview to a real 390px viewport. Renders in `app` and pre-login `auth` modes | — | App mode: Dashboard, Roadmap. Auth mode: Invite Redeem, Login, Roadmap |
...
| Login | Page | Default for unauthenticated users | Email + password sign in. Includes "Forgot password?" link, switch to invite redeem, and a "Take the tour" button (no account required). The "Explore the live demo" button is temporarily hidden (`SHOW_DEMO_CTA` flag in `LoginPage.tsx`) — the tour is the primary public entry point; the demo remains reachable via the tour's embedded previews and `/?demo=1` | — | Invite Redeem, Forgot Password, Take a Tour |
...
Entered via `/?demo=1` (the tour's embedded live previews run on the same mechanism);
exited via the banner or the user menu ("Exit demo"). The Login screen's "Explore the
live demo" button is temporarily hidden behind the `SHOW_DEMO_CTA` flag in
`LoginPage.tsx`. Not a separate set of screens — the entire app renders normally against
seeded demo data, with three chrome additions:
