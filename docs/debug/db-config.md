...
- **Invariant:** At most one habit entry document per `(householdId, userId, habitId, dayKey)`. Soft-delete sets `deletedAt` on that same document; we never create a second document for the same key.
- **Index:** A **unique** index is created at startup in `src/server/lib/mongoClient.ts` (ensureCoreIndexes):
  - Keys: `{ householdId: 1, userId: 1, habitId: 1, dayKey: 1 }`
...
- **Duplicate handling:** Before creating the index, the server runs an aggregation over every document using the exact full index fields. If any exist, it logs a warning and skips this index while continuing other index assurance. The migration archives and physically removes losers because a soft-deleted loser still conflicts with a full unique index. Run backfill/dedupe, verify, then restart.
