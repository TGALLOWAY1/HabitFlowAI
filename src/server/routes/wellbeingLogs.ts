...
export async function upsertWellbeingLogRoute(req: Request, res: Response): Promise<void> {
  try {
    ...
    // NOTE: Dual-write adapter removed (C3 audit fix).
    // Frontend now reads/writes canonical wellbeingEntries directly.
    // This legacy endpoint writes to wellbeingLogs only.
